-- TODO: Dummy data. Remover antes de desplegar
INSERT INTO users (name, rut, email, password)
VALUES
    (
        'user_1',
        '1.111.111-1',
        'user_1@crececonsultores.cl',
        '$2a$12$yzHIYT5pIdYkQ6CmnGhKluV2oQPgIThlLa7D9Z0xwN.YqrcBrOBsy'
    ),
    (
        'user_2',
        '2.222.222-2',
        'user_2@crececonsultores.cl',
        '$2a$12$NamZAphLz9jQb3c28nCjae24VdmcbFM9pgy//gKfZRrvwDhniPpFC'
    ),
    (
        'user_3',
        '3.333.333-3',
        'user_3@crececonsultores.cl',
        '$2a$12$.s8kh8u8WWA6JYwRqcuY4OUdoCFPzJ26UW.h.qQ6i7l/xInBYsrcW'
    );

INSERT INTO clients (name, rut, companyRut, email, password)
VALUES
    (
        'client_1',
        '4.444.444-4',
        '1.111.111-K',
        'client_1@company1.cl',
        '$2a$12$FlZJ2mTZwhDyz/eL2fumTeNRESGjBTYCkjia7yTahwc9wmWW.l0wu'
    ),
    (
        'client_2',
        '5.555.555-5',
        '2.222.222-K',
        'client_2@company2.cl',
        '$2a$12$xy8eo3SwydfJ/Ce1Nn1wz.S2fqGWyTnchDH.0YyqXGJdxe1jNOBsS'
    ),
    (
        'client_3',
        '6.666.666-6',
        '3.333.333-K',
        'client_3@company3.cl',
        '$2a$12$D5ztlO8WoiOsxiLdlJHyGuNgOwsGGHy0E6ONXzc45CrUHKoJpCLE.'
    );